...
| 类型 | 形态 | 处理 |
|---|---|---|
| 贴片 logo（云图标 + `LCEDA` + `EasyEDA`） | 被拆成 **13 个零体积平面小片**贴在外壳上 | 判据「Z 向厚度 ≤ 0.02mm + 体积 ≤ 0.02mm³ + 该 solid 全部面都很亮 + 命中 ≥ 2 片」→ 直接从 `Solids` 里删掉 |
| 本体刻字（如 `SOIC16`） | 刻进本体的凹槽，深约 0.01mm，删不掉 | 把「顶面 0.03mm 薄层内 + 比本体亮 + XY 上分成 ≥ 4 个独立小块」的面**改成本体主色**，字就隐形（几何仍在，10µm 深，渲染/打印无影响） |

两条判据都带 `≥2` / `≥4` 块的保护条件，所以**单片的白色 1 脚标记会被保留**。
